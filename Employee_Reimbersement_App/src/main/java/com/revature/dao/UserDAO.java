...
public class UserDAO {
	...
	public User addUsers(AddOrUpdateUsersDTO user) throws SQLException {
		...
		try (Connection conn = JDBCUtility.getConnection()) {
			String sql = "INSERT INTO emp_reimbersement.users(user_username,user_password,user_first_name,user_last_name,user_email,user_role)VALUES(?,?,?,?,?,?)";
	...
	public User getUserByUsernameAndPassword(String username, String password) throws SQLException {
		try (Connection conn = JDBCUtility.getConnection()) {
			String sql = "select* from emp_reimbersement.users  WHERE email = ? AND password = ?;";
